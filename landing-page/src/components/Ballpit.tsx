...
function createBallpit(canvas: HTMLCanvasElement, config: any = {}): CreateBallpitReturn {
  ...
  const pointerData = createPointerData({
    domElement: canvas,
    onMove() {
      if ((spheres.config as any).ignoreCursor) return;
...
export interface BallpitHandle {
  ...
  dropCursorBalloon: () => void;
  restoreCursorBalloon: () => void;
}

const Ballpit = React.forwardRef<BallpitHandle, BallpitProps>(({ className = '', followCursor = true, ...props }, ref) => {
  ...
  React.useImperativeHandle(ref, () => ({
    setGravity: (g: number) => {
      if (spheresInstanceRef.current) {
        spheresInstanceRef.current.spheres.physics.config.gravity = g;
      }
    },
    dropCursorBalloon: () => {
      if (spheresInstanceRef.current) {
        const physics = spheresInstanceRef.current.spheres.physics;
        // Ignore cursor movements
        (spheresInstanceRef.current.spheres.config as any).ignoreCursor = true;
        // Disable following cursor
        spheresInstanceRef.current.spheres.config.controlSphere0 = false;
        // Give it a strong downward velocity
        physics.velocityData[1] = -2.5; 
      }
    },
    restoreCursorBalloon: () => {
      if (spheresInstanceRef.current) {
        (spheresInstanceRef.current.spheres.config as any).ignoreCursor = false;
        spheresInstanceRef.current.spheres.config.controlSphere0 = true;
      }
    }
  }));
